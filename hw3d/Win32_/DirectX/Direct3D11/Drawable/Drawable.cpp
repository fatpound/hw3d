...
{
    assert((typeid(*bind) != typeid(IndexBuffer)) && "Must use AddIndexBuffer_ to bind index buffer");

    binds_.push_back(std::move(bind));
}

void Drawable::AddIndexBuffer_(std::unique_ptr<IndexBuffer> ibuf) noexcept(IN_RELEASE)
{
